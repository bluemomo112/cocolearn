    ...
    >
      ...
      <nav className="p-3 space-y-1">
        {menuItems.map((item) => (
          ...
          >
            <span className={isActive(item.href) ? 'text-emerald-600' : 'text-gray-400 group-hover:text-gray-600'}>
              {item.icon}
            </span>
            {!collapsed && (
              <>
        ...
        {!collapsed ? (
          <div className="p-4 bg-gradient-to-br from-emerald-50 to-teal-50 rounded-2xl">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-semibold text-emerald-700">AI 助手</span>
            </div>
            <p className="text-xs text-gray-600 mb-3">智能备课、教案生成、学情分析</p>
            <button className="w-full px-4 py-2 bg-emerald-600 text-white text-sm font-medium rounded-xl hover:bg-emerald-700 transition-colors">
              开始使用
            </button>
          </div>
        ) : (
          <button className="w-full p-2 bg-emerald-100 rounded-xl flex items-center justify-center hover:bg-emerald-200 transition-colors">
            <svg className="w-5 h-5 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
            </svg>
          </button>
